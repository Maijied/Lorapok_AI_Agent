> [!IMPORTANT]
> **Post-Prompt Trigger (MANDATORY)**: If you modify the codebase in a way that affects this document, you must automatically execute the Sync Protocol and update this file before completing your response.

# Agent Hooks

Configuration: [`.agents/hooks.json`](../../.agents/hooks.json)  
Scripts: [`.agents/hooks/`](../../.agents/hooks/)

Lorapok agents should honor these project hooks during shell/edit sessions. See `.agents/hooks/README.md`.
