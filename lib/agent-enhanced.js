...
4. Ensure paths are relative to project root.`;
    ...
    parseActions(content) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (line.match(/^ACTION:\s*(CREATE|UPDATE|DELETE)\s*FILE:\s*/i)) {
                const actionMatch = line.match(/ACTION:\s*(\w+)\s*FILE:\s*(.+)/i);
                if (actionMatch) {
                    ...
                    // DELETE actions don't need a code block, so reset immediately
                    if (type.toUpperCase() === 'DELETE') currentAction = null;
                }
            } else if (line.match(/^ACTION:\s*RUN\s*COMMAND:\s*/i)) {
                const actionMatch = line.match(/ACTION:\s*RUN\s*COMMAND:\s*(.+)/i);
                if (actionMatch) {
                    const [_, description] = actionMatch;
                    currentAction = {
                        type: 'COMMAND',
                ...
                // If we hit a code block, capture the content
                if (line.startsWith('```')) {
                    let codeLines = [];
                    i++; // skip opening ```
                    ...
                    currentAction = null; // Important: Stop looking for more code blocks for this specific action
                }
            }
        }
        return actions;
