...
class LorapokEnhancedAgent extends LorapokCodingAgent {
    ...
    constructor(apiKey, projectRoot = process.cwd()) {
        ...
        this.indexer = new IndexerService({ projectRoot });
    ...
    /**
     * Start background services like file watching and indexing
     */
    async initServices() {
        await this.indexer.init();
        this.indexer.startWatching();
        // Fire and forget initial index if needed, or let it index lazily
    }
...
4. Ensure paths are relative to project root.`;
    ...
    /**
     * Override chat to assemble intelligent context using LanceDB and Tree-sitter.
     */
    async chat(userMessage, model = null, context = {}) {
        if (!context.assembledContext) {
            // Only assemble if not already assembled by a specialized method
            const planFiles = context.planFiles || [];
            context.assembledContext = await this.contextAssembler.assemble(userMessage, planFiles);
        }
    ...
    parseActions(content) {
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            const fileActionRegex = /^(?:\*\*|`|>)*ACTION:\s*(CREATE|UPDATE|WRITE|EDIT|MODIFY|DELETE)(?:_|\s*)?(?:FILE)?:?\s*[`*]*([^\s`*]+)/i;
            const commandActionRegex = /^(?:\*\*|`|>)*ACTION:\s*(?:RUN|EXECUTE|BASH|SHELL|COMMAND)(?:_|\s*)?(?:COMMAND|BASH|SHELL)?:?\s*(.+?)(?:\*\*|`)*$/i;

            const fileMatch = line.match(fileActionRegex);
            if (fileMatch) {
                let [_, typeRaw, filePath] = fileMatch;
                let type = typeRaw.toUpperCase();
                if (type === 'WRITE' || type === 'EDIT' || type === 'MODIFY') type = 'UPDATE';
                ...
                if (type === 'DELETE') currentAction = null;
            } else {
    ...
    /**
     * View file content with line range slicing.
    ...
    viewFile(filePath, options = {}) {
        return this.fileManager.viewFile(filePath, options);
    }
    ...
    replaceFileContent(filePath, targetContent, replacementContent, options = {}) {
        return this.fileManager.replaceFileContent(filePath, targetContent, replacementContent, options);
    }

    /**
     * Multiple non-contiguous string replacements in a single file.
     * @param {string} filePath - Target file path
    ...
    multiReplaceFileContent(filePath, chunks = []) {
        return this.fileManager.multiReplaceFileContent(filePath, chunks);
    }
     ...
     * @returns {{ success: boolean, data?: Array<Object>, error?: string }} Results
     */
    grepSearch(query, searchPath = '.', options = {}) {
        return this.fileManager.grepSearch(query, searchPath, options);
    }
     ...
     * @param {Object} [options={}] - Filter options
     * @returns {{ success: boolean, data?: Array<Object>, error?: string }} Directory list
     */
    listDir(dirPath = '.', options = {}) {
        return this.fileManager.listFiles(dirPath, options);
    }
...
Provide a detailed "Walkthrough" of what was accomplished and how the user can verify it.`;
    ...
    async analyzeProject(context = {}) {
        // Use the context assembler to get a high-level summary and structurally significant files
        // by pretending the user asked for a full architectural review.
        const assembledContext = await this.contextAssembler.assemble('Analyze project architecture, dependencies, and structure.', [], 16000);
...
4. **Roadmap**: Immediate potential improvements.`;
        ...
        return this.chat(prompt, null, { ...context, task: 'project-analysis', assembledContext });
    }
}

module.exports = { LorapokEnhancedAgent };