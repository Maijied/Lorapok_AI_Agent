...
const MODELS = {
    ...
    'gemini-3.1-pro': { name: '✨ Gemini 3.1 Pro (Google AI Studio)', tier: 'pro', provider: 'google-ai-studio' },
    ...
    'gemini-2.5-flash': { name: '⚡ Gemini 2.5 Flash (Google AI Studio)', tier: 'free', provider: 'google-ai-studio' },
    ...
    'gemini-1.5-pro': { name: '🎯 Gemini 1.5 Pro (Google AI Studio)', tier: 'pro', provider: 'google-ai-studio' },
...
class LorapokCodingAgent {
    ...
    async checkAvailableModels() {
        ...
        const results = await this.modelManager.fetchModels();
        ...
        for (const modelId of Object.keys(results)) {
            const provider = results[modelId]?.provider || 'perplexity';
            if (provider === 'google-ai-studio') {
                results[modelId].available = Boolean(googleKey && googleKey.trim() !== '');
            } else if (provider === 'openrouter') {
                results[modelId].available = Boolean(openRouterKey && openRouterKey.trim() !== '');
            } else {
                if (!perplexityKey) {
                    results[modelId].available = false;
                    continue;
                }
                try {
                    const payload = {
                    ...
                    results[modelId].available = true;
                } catch (e) {
                    results[modelId].available = false;
                }
            }
    ...
    async callPerplexityAPI(messages, model, options = {}) {
        ...
        try {
            ...
            if (status === 429) {
                const provName = isGoogle ? 'Google AI Studio' : (isOpenRouter ? 'OpenRouter' : 'Perplexity');
                throw new Error(`${provName} API Rate Limit / Quota Exceeded (${status}): ${message}`);
            }
            if (status === 404) {
