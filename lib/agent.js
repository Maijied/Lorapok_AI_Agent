    ...
    async callPerplexityAPI(messages, model, options = {}) {
        ...
        try {
            ...
            if ((status === 429 || status === 404 || isModalityError) && !options.isFallbackAttempt) {
                ...
                if (mappedModel === 'gemini-3.6-flash') fallbackModel = 'gemini-3.5-flash-lite';
                if (isOpenRouter) fallbackModel = 'meta-llama/llama-3.3-70b-instruct';
                if (isPerplexity) fallbackModel = 'sonar';

                if (fallbackModel !== mappedModel) {
                    logger.warn(`API call failed for '${mappedModel}' (${status || 'Modality Error'}). Automatically routing fallback to '${fallbackModel}'...`);
                    console.log(chalk.yellow(`\n⚠️ Model '${mappedModel}' ${status === 429 ? 'quota exceeded / rate limited' : isModalityError ? 'modality unsupported' : 'unavailable'}.`));
