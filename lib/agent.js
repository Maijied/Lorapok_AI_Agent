    ...
    async callPerplexityAPI(messages, model, options = {}) {
        ...
        try {
            ...
            if (status === 429) {
                ...
                let retryNotice = '';
                if (retryHeader) {
                    retryNotice = ` [Retry in ${retryHeader}s]`;
                }
                const resetInfo = isGoogle ? ' (Quotas reset every 1m for RPM or 00:00 UTC for RPD)' : ' (Rate limits reset per minute/daily)';
                throw new Error(`${provName} API Rate Limit / Quota Exceeded (${status})${retryNotice}: ${message}\n💡 Tip:${resetInfo}. Switch model or wait briefly.`);
            }
            if (status === 404) {
