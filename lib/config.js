...
class LorapokConfig {
  ...
  getMaxCostUsd() {
    return Number(this.config.maxCostUsd) || 0;
  }

  setMaxCostUsd(cost) {
    this.set('maxCostUsd', Number(cost) || 0);
  }

  getMaxTokens() {
    return Number(this.config.maxTokens) || 0;
  }

  setMaxTokens(tokens) {
    this.set('maxTokens', Number(tokens) || 0);
  }

  getMaxToolCalls() {
    // Default to 25 if not set
    return this.config.maxToolCalls !== undefined ? Number(this.config.maxToolCalls) : 25;
  }

  setMaxToolCalls(calls) {
    this.set('maxToolCalls', Number(calls) || 25);
  }
