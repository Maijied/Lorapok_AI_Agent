...
/**
 * Professional farewell emblem for SESSION RECAP (right of METRICS).
 * Cleaner cyber-larva: soft antennae, code badge, animated eyes + "bye bye".
 * Inspired by the Slant/cyber larva mark, tightened for CLI chrome.
...
function getByeByeEmblemLines(frame = 0) {
    ...
    const wave = f % 2 === 0 ? `  /| ${Up}\u2570\u256F${Up} |\\ ` : `  \\| ${Up}\u2570\u256F${Up} |/ `;

    return rows(
        antennae,
        `  ${Lo}${B}${B}${B}${B}${B}${B}${B}${Lo} `,
        ` ${B}${eyes[0]}${B}${B}${B}${B}${B}${eyes[1]}${B}`,
        ` ${Lf}\u2591${D}${D}{}${D}${D}\u2591${Rt}`,
        ` ${B}${B}${B}${B}${B}${B}${B}${B}${B}`,
...
/**
 * Theme-colored bye-bye emblem lines for recap sidebar.
 * @param {number} [frame=0]
...
function renderByeByeEmblem(frame = 0, theme = null) {
    ...
    return colored.map((line, idx) => {
        ...
        if (/bye/.test(plain)) return info(plain);
        if (plain.includes('>_code;')) {
            // Keep badge readable: accent the prompt glyph
            return line.replace('>_code;', accent('>_code;'));
        }
        return line;
    });
}
