...
function createCodeBox(code, lang = '') {
    ...
    const labelTotalWidth = labelText.length + 4;
    const lineBefore = '─'.repeat(2);
    const lineAfter = '─'.repeat(Math.max(0, width - labelTotalWidth - 2));
    ...
    lines.forEach((line, i) => {
        ...
        // Dynamic wrapping or truncation check
        let processedLine = line;
        const maxCodeWidth = width - 10;
