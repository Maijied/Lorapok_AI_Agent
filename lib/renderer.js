...
function createCodeBox(code, lang = '') {
    ...
    const width = Math.min(process.stdout.columns - 6 || 76, 90);
    const lines = code.trim().split('\n');

    // High-contrast colors for dark terminals
    const isShell = ['bash', 'sh', 'shell', 'terminal'].includes(lang.toLowerCase());
    ...
    const labelText = ` ${displayLang} `;
    const remainingWidth = Math.max(0, width - labelText.length - 4);
    output += borderChar('  ┌─') + labelColor(labelText) + borderChar('─'.repeat(remainingWidth)) + borderChar('┐') + '\n';
    ...
    lines.forEach((line, i) => {
        ...
        const padding = ' '.repeat(Math.max(0, width - truncated.length - 8));
        output += borderChar('  │') + lineNum + codeColor(truncated) + padding + borderChar('│') + '\n';
    });
...
function renderMarkdownSync(text) {
    ...
    result = result.replace(tableRegex, (match) => {
        try {
            ...
            if (colCount > 5 || (colCount > 3 && termWidth < 90)) {
                let cardView = '\n';
            ...
            const colWidth = Math.max(15, Math.floor((termWidth - 10) / colCount));
