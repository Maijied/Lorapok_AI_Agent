...
function createCodeBox(code, lang = '') {
    ...
    const labelTotalWidth = labelText.length + 4; // 1(┌) + 2(──) + 1(┐) + label
    const lineBefore = '─'.repeat(2);
    // Top border math: 1(┌) + 2(before) + label + lineAfter + 1(┐) = width
    ...
    lines.forEach((line, i) => {
        ...
        // Width math: 1(│) + 6(num) + code + 1(space) + 1(│) = width
        // code = width - 9
        ...
        // Note: suffix is ' │' (space + pipe) to match the width calc
