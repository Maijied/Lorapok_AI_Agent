    ...
    try {
        ...
        marked.use(TerminalRenderer({
            ...
            blockquote: (quote) => {
                ...
                return chalk.gray('  │ ') + chalk.whiteBright.italic(q.replace(/\n/g, '\n  │ '));
            },
            html: chalk.reset,
            heading: (text, level) => {
                const t = String(text || '').trim();
                if (level <= 2) {
                    return '\n' + chalk.cyanBright.bold(`  ${t}`) + '\n' + chalk.gray('  ' + '─'.repeat(Math.min(48, termW - 6))) + '\n';
                }
                return '\n' + chalk.cyan.bold(`  ${t}`) + '\n';
            },
            firstHeading: (text) => {
            ...
            codespan: (text) => chalk.yellowBright(`\`${text}\``),
            ...
            emoji: true,
            tab: 2,
            listitem: (text) => chalk.cyan('    ▶ ') + chalk.whiteBright(String(text || '').replace(/^\s*[•\-*]\s*/, '')),
            list: (body) => '\n' + body + '\n'
        }));
