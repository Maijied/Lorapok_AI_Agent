...
class TerminalUI {
    ...
    /**
     * Full exit flow: animated shutdown → professional SESSION RECAP → motto.
    ...
    static showInteractionSummary(sessionData, options = {}) {
        ...
        const metricLines = rows.map(([label, value]) =>
            '  ' + padEndVisible(label, labelW) + '  ' + padStartVisible(value, valueW)
        );

        const rule = theme.muted('  ' + '─'.repeat(labelW + valueW + 2));
        // Visual break before totals
        metricLines.splice(3, 0, rule);

        let body = theme.color('info', '  METRICS') + '\n' + metricLines.join('\n');
        ...
        if (!viewOnly) {
            ...
            if (canAnimateExit() && !options.skipAnimation) {
                // Brief final pulse on the motto line context
