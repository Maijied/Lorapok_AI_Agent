...
class TerminalUI {
    ...
    static showHeader(version = '1.0.0') {
        console.clear();
        const logo = this.getLogo();
        console.log(logo);
        console.log(chalk.gray(`  ${'═'.repeat(40)}`));
        console.log(chalk.gray(`  v${version} 🐛 EXPERT CODING AGENT`) + '\n');
    }
    ...
    static showHelp() {
        ...
        table.push(
            ['/chat', 'Standard conversation mode', 'Enter'],
            ['/plan', 'Planning -> Task -> Implementation', '-'],
            ['/analyze', 'Analyze project architecture', '-'],
            ['/logs', 'View the latest error logs', '-'],
            ['/files', 'List and tree file operations', '-'],
            ['/git', 'Commit, push, branch management', '-'],
            ['/settings', 'Change model / language / key', '-'],
    ...
    static hideLongCodeBlocks(content, threshold = 15) {
