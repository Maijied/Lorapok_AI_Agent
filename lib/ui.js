...
class TerminalUI {
    ...
    static buildSessionRecapBox(sessionData, theme, frame = 0) {
        ...
        // Band width matches model table so compact logo sits in the right gutter
        const bandInnerW = Math.max(leftW + 2 + byeW, modelBlockW - 2, leftW + 14);
        ...
        for (let i = 0; i < bandH; i++) {
            ...
            const R = (byeIdx >= 0 && byeIdx < byeLines.length)
                ...
                : ' '.repeat(byeW);
            const midPad = Math.max(2, bandInnerW - leftW - byeW);
            band.push('  ' + L + ' '.repeat(midPad) + R);
        }
