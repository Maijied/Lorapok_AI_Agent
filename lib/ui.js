...
class TerminalUI {
    static getBranding(font = 'Slant', bugFrame = 0, version = '1.0.0') {
        // Fallback if font is invalid or empty
        if (!font) font = 'Slant';

        // Map "Unique Stack" themes to actual small/appropriate fonts
        const fontMapping = {
            'Roman': 'Small Shadow',    // Smaller version of Roman
            'Big': 'Standard',          // Standard is cleaner than Big
            'Graceful': 'ANSI Shadow',  // Professional & Clean
            'Cyberlarge': 'Small',      // Digital/Hacker vibe but small
            'Straight': 'Small Slant',  // AI Researcher vibe
            'Executive': 'Calvin S',    // High-end look
            'Engineering': 'Delta Corps Priest 1' // Technical look
        };
        ...
        let text1, text2;
        try {
            text1 = figlet.textSync('LORAPOK', { font: actualFont });
            text2 = figlet.textSync('CLI', { font: actualFont });
        } catch (e) {
            // Fallback to Standard/Small and retry
        ...
        const space = "      "; // Reduced space for split layout
        const brandingLines = [];

        for (let i = 0; i < maxH; i++) {
            const part1 = (trimmed1[i] || '').padEnd(maxW1, ' ');
            const part2 = trimmed2[i] || '';
            brandingLines.push(chalk.cyan.bold(part1 + space + part2));
        }

        // Add Metadata
        const welcomeLine = chalk.white("   Welcome to Lorapok");
        const versionLine = chalk.gray(`   CLI Version ${version}`);
        ...
        // --- PREPARE RIGHT COLUMN (GRAPHIC) ---
        const rawLarvaLines = LARVA_FRAMES[bugFrame % LARVA_FRAMES.length];
        // Center graphic vertically relative to text block if needed, or just top align
        // The text block is likely taller or similar height. 
        // Larva is ~9 lines. Text block with metadata is ~10-12 lines depending on font.

        const larvaColor = chalk.hex('#A020F0'); // Purple-ish like the ghost
        const larvaLines = rawLarvaLines.map(l => larvaColor(l));

        // --- MERGE COLUMNS ---
        const leftWidth = Math.max(...leftBlock.map(l => l.replace(/\u001b\[\d+m/g, '').length));
        const totalRows = Math.max(leftBlock.length, larvaLines.length);

        // Calculate offsets for vertical centering
        const leftOffset = Math.floor((totalRows - leftBlock.length) / 2);
        const rightOffset = Math.floor((totalRows - larvaLines.length) / 2);

        let contentRows = [];
        for (let i = 0; i < totalRows; i++) {
            // Get content for Left column (with offset)
            let rawLeft = '';
            if (i >= leftOffset && i < leftOffset + leftBlock.length) {
                rawLeft = leftBlock[i - leftOffset] || '';
            }

            const rawLeftLen = rawLeft.replace(/\u001b\[\d+m/g, '').length;
            const padLen = leftWidth - rawLeftLen;
            const paddedLeft = rawLeft + ' '.repeat(padLen);

            // Get content for Right column (with offset)
            let right = '';
            if (i >= rightOffset && i < rightOffset + larvaLines.length) {
                right = larvaLines[i - rightOffset] || '';
            }

            contentRows.push(paddedLeft + "     " + right); // 5 spaces gap
        }

        // --- CREATE CUSTOM FRAME ---
        const contentWidth = Math.max(...contentRows.map(l => l.replace(/\u001b\[\d+m/g, '').length));
        const topFrame = chalk.gray('┌' + '─'.repeat(contentWidth + 2) + '┐');
        const bottomFrame = chalk.gray('└' + '─'.repeat(contentWidth + 2) + '┘');
