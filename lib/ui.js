...
class TerminalUI {
    static getBranding(font = 'Slant', bugFrame = 0) {
        ...
        const space = "        "; // 8 spaces separation
        const textLines = [];

        for (let i = 0; i < maxH; i++) {
            const part1 = (trimmed1[i] || '').padEnd(maxW1, ' ');
            const part2 = trimmed2[i] || '';
            textLines.push(chalk.cyan.bold(part1 + space + part2));
        }

        const maxTextLen = Math.max(...textLines.map(l => l.replace(/\u001b\[\d+m/g, '').length));

        const bugLines = BUG_FRAMES[bugFrame % BUG_FRAMES.length];
        const maxBugLen = Math.max(...bugLines.map(l => l.length));
        const totalWidth = Math.max(maxTextLen, maxBugLen);

        const centeredBug = bugLines.map(line => {
            const padding = Math.max(0, Math.floor((totalWidth - line.length) / 2));
            return ' '.repeat(padding) + chalk.yellow.bold(line);
        }).join('\n');

        const centeredText = textLines.map(line => {
            // Strip ansi to get length for centering
            const len = line.replace(/\u001b\[\d+m/g, '').length;
            const padding = Math.max(0, Math.floor((totalWidth - len) / 2));
            return ' '.repeat(padding) + line;
        }).join('\n');

        const brandingContent = centeredBug + '\n\n' + centeredText;

        return boxen(brandingContent, {
     ...
     */
    static async animateLogo(duration = 1500, font = 'Slant') {
        ...
        return new Promise(resolve => {
            const timer = setInterval(() => {
                ...
                console.log(this.getBranding(font, frameIdx));
                i++;
                if (i >= totalSteps) {
                    clearInterval(timer);
    ...
    static async previewThemes(config) {
        ...
        if (choice) {
            ...
            console.clear();
            console.log(this.getBranding(choice));
