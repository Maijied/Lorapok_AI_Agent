...
class TerminalUI {
    ...
    static buildSessionRecapBox(sessionData, theme, frame = 0) {
        ...
        const labelW = 14;
        const valueW = 14;
        const leftW = labelW + valueW + 6; // "  " + label + "  " + value
        ...
        const metricRows = [
            ...
            [theme.color('info', 'Total tokens'), theme.color('info', fmt(data.tokens?.total))]
        ];
        ...
        const gap = '   ';
        const bandH = Math.max(leftLines.length, byeLines.length);
        const band = [];
        for (let i = 0; i < bandH; i++) {
            const L = padEndVisible(leftLines[i] || '', leftW);
            const R = padEndVisible(byeLines[i] || '', byeW);
            band.push('  ' + L + gap + R);
        }
