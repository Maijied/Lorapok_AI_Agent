...
function padStartVisible(str, width) {
    const s = String(str ?? '');
    return ' '.repeat(Math.max(0, width - stripAnsi(s).length)) + s;
}
...
function canAnimateExit() {
    return Boolean(
        process.stdout.isTTY &&
        !process.env.CI &&
        process.env.LORAPOK_NO_ANIM !== '1' &&
        process.env.LORAPOK_NO_ANIM !== 'true'
    );
}
...
function withGutter(text, gutter = 2) {
    const pad = ' '.repeat(Math.max(0, gutter));
    return String(text || '')
        .split('\n')
...
function formatPathShort(pathStr, maxLen = 36) {
    ...
    if (parts.length === 1 && parts[0].length + 2 <= maxLen) return `…/${parts[0]}`;
...
class TerminalUI {
    ...
    static getBranding(font = 'Lorapok', bugFrame = 0, version = '1.0.0', model = '', pathStr = '', logoStyleOrConfig = null) {
        ...
        const pathShort = formatPathShort(pathStr, 36);
        ...
        if (isClassic) {
            ...
            const larvaStart = Math.max(larvaPad, aiW + badgeW + 4);
            ...
            // Theme label left-aligned under the hero (not floating under larva)
            header.push('');
            header.push(themeLabel);
            heroRows = header;
        } else {
            ...
            for (let i = 0; i < bandH; i++) {
                ...
                heroRows.push(`${left}${' '.repeat(gap)}${divider}${' '.repeat(gap)}${right}${rightPad}`);
            }
            heroRows.push('');
            heroRows.push(themeLabel);
        }

        const heroW = Math.max(...heroRows.map(l => stripAnsi(l).length), stripAnsi(metaLine).length, 48);
        const rule = theme.muted((theme.frame.h || '\u2500').repeat(heroW));
        const framed = frameLines([...heroRows, rule, metaLine], theme);

        const welcomeInner =
            theme.color('info', 'Welcome to Lorapok AI') +
            theme.muted('  ·  ') +
            theme.muted(MOTTO);
        // Same frame builder + content width as hero → borders align perfectly
        const welcomeBox = frameLines([padEndVisible(welcomeInner, heroW)], theme);

        return withGutter(welcomeBox, GUTTER) + '\n' + withGutter(framed, GUTTER) + '\n';
    }
    ...
    static showWelcome(config = null) {
        ...
        const g = '  ';
        console.log(
            g + theme.muted('lorapok.tech') +
            theme.muted('  ·  ') + theme.muted('type ') +
            theme.color('text', '/help') + theme.muted(' for commands')
        );
        console.log(
            g + theme.color('text', '/') + theme.muted(' commands') +
            theme.muted('  ·  ') +
            theme.color('text', '@') + theme.muted(' files') +
            theme.muted('  ·  ') +
            theme.color('text', 'Ctrl+C') + theme.muted(' ×2 to exit\n')
        );
    }
    ...
    static showHelp(config = null) {
        ...
        const legendLines = ModelManager.getTierLegend().map(row => {
            const paint = chalkByColor[row.color] || chalk.white;
            return `  ${paint(row.icon)} ${paint(row.title.padEnd(26))} ${chalk.gray(row.detail)}`;
        }).join('\n');

        const legendBlock =
            chalk.bold('\nMODEL STATUS LEGEND (menus / /model / /settings)\n') +
            chalk.gray('  Colors and icons mark access level — not “broken vs working.”\n') +
            legendLines +
            chalk.gray('\n\n  Menus: Currently Usable = no-payment + keyed. Paid Catalog = credits/locked.\n') +
            chalk.gray('  Run /refresh-models to live-probe; 🔴 means quota is hot — pick 🟢 Flash or wait.\n') +
            chalk.bold('\nRESPONSE VIEW\n') +
            chalk.greenBright('  H1') + chalk.gray('  green + double rule  ·  ') +
            chalk.cyanBright('H2') + chalk.gray('  cyan + single rule  ·  ') +
            chalk.cyan('H3') + chalk.gray('  ▎ accent  ·  ') +
            chalk.magentaBright('H4') + chalk.gray('  ▸ accent\n') +
            chalk.whiteBright.bold('  **bold**') + chalk.gray(' labels  ·  ') +
            chalk.yellowBright('inline code') + chalk.gray('  ·  ') +
            chalk.gray('·') + chalk.gray(' list bullets  ·  numbered lists use cyan indexes\n') +
            chalk.gray('  Frame: LORAPOK · response with full-width rule; body soft-wrapped to the panel.\n') +
            chalk.gray('  Modes: Chat, Plan, Agent, Debug/Analyze. Lorapok uses phrasing inference to pick a mode if not explicit.\n');

        console.log(theme.box(
            theme.color('info', 'LORAPOK COMMAND REFERENCE') + '\n\n' + table.toString() + legendBlock,
    ...
    static showHowToUse() {
        ...
        content += `${chalk.gray('  • Status colors (also in ')}${chalk.cyan('/help')}${chalk.gray('):')}\n`;
        {
            const { ModelManager } = require('../services/ModelManager');
            const chalkByColor = {
                green: chalk.green, cyan: chalk.cyan, magenta: chalk.magenta,
                red: chalk.red, yellow: chalk.yellow, gray: chalk.gray
            };
            for (const row of ModelManager.getTierLegend()) {
                const paint = chalkByColor[row.color] || chalk.white;
                content += `    ${paint(row.icon)} ${paint(row.title)} ${chalk.gray('— ' + row.detail)}\n`;
            }
        }
        content += '\n';
        ...
        content += `${chalk.yellow.bold('5. 📝 Operating Modes & Phrasing Inference')}\n`;
        content += `${chalk.gray('  • Lorapok operates in 4 core modes: ')}${chalk.cyan('Chat')}${chalk.gray(', ')}${chalk.cyan('Plan')}${chalk.gray(', ')}${chalk.cyan('Agent')}${chalk.gray(', and ')}${chalk.cyan('Debug/Analyze')}${chalk.gray('.')}\n`;
        content += `${chalk.gray('  • Use explicit commands (' )}${chalk.cyan('/chat, /plan, /agent, /debug')}${chalk.gray(') or let Lorapok infer the mode from your phrasing.')}\n`;
        content += `${chalk.gray('    - "plan to refactor..." -> Plan mode')}\n`;
        content += `${chalk.gray('    - "fix this bug..." -> Agent/Debug mode')}\n`;
        content += `${chalk.gray('    - "explain this..." -> Chat mode\n\n')}`;
    ...
    /**
     * Animated shutdown steps before SESSION RECAP (TTY only; skipped in CI / non-interactive).
     * @param {Object} [options]
     * @param {string} [options.themeId]
     * @param {boolean} [options.skipAnimation]
     * @returns {Promise<void>}
     */
    static async animateExitSequence(options = {}) {
        const theme = getTheme(options.themeId || getDefaultThemeId());
        if (options.skipAnimation || !canAnimateExit()) {
            console.log(theme.muted('\n  Closing session…\n'));
            return;
        }

        const spinner = theme.applyToOra(ora({
            text: theme.muted('Closing session…'),
            spinner: { interval: 90, frames: getLarvaSpinnerFrames() },
            discardStdin: false
        }));
        spinner.start();

        const steps = [
            'Flushing conversation metrics…',
            'Writing session recap…',
            'Archiving workspace state…',
            'Shutting down agent…'
        ];
        for (const step of steps) {
            spinner.text = theme.muted(step);
            await sleepMs(320);
        }
        spinner.succeed(theme.success('Session closed'));
        console.log('');
    }

    /**
     * Full exit flow: animated shutdown → SESSION RECAP with bye-bye larva → motto.
     * @param {Object} sessionData
     * @param {Object} [options]
     * @returns {Promise<void>}
     */
    static async exitSession(sessionData, options = {}) {
        await TerminalUI.animateExitSequence(options);
        if (!options.skipAnimation && canAnimateExit()) {
            await TerminalUI.animateSessionRecap(sessionData, options);
        } else {
            TerminalUI.showInteractionSummary(sessionData, { ...options, viewOnly: false, afterAnimation: true });
        }
    }

    /**
     * Animate SESSION RECAP frames (bye-bye larva waves) then settle on final frame.
     * @param {Object} sessionData
     * @param {Object} [options]
     * @returns {Promise<void>}
     */
    static async animateSessionRecap(sessionData, options = {}) {
        const theme = getTheme(options.themeId || getDefaultThemeId());
        let lastLines = 0;
        const frames = 6;
        for (let f = 0; f < frames; f++) {
            const boxed = TerminalUI.buildSessionRecapBox(sessionData, theme, f);
            if (f > 0 && process.stdout.isTTY) {
                process.stdout.write(`\x1b[${lastLines}A\x1b[J`);
            }
            console.log(boxed);
            lastLines = String(boxed).split('\n').length + 1;
            await sleepMs(160);
        }
        console.log(theme.muted('\n  ') + theme.color('text', MOTTO));
        console.log(theme.success('  ✓  Ready for the next session.\n'));
    }

    /**
     * Build SESSION RECAP boxen string (metrics left, bye-bye larva right).
     * @param {Object} sessionData
     * @param {Object} theme
     * @param {number} [frame=0]
     * @returns {string}
     */
    static buildSessionRecapBox(sessionData, theme, frame = 0) {
        const data = sessionData || {};
        // Compact metrics: tight label/value columns (no huge empty gap)
        const labelW = 12;
        const valueW = 10;
        const leftW = labelW + 2 + valueW;
        const fmt = (n) => Number(n || 0).toLocaleString();

        const metricRows = [
            [theme.muted('Session'), theme.color('text', String(data.id || '—'))],
            [theme.muted('Turns'), theme.color('text', String(data.count ?? 0))],
            [theme.muted('Success'), theme.success(`${data.successRate ?? 0}%`)],
            null,
            [theme.muted('Prompt'), theme.muted(fmt(data.tokens?.prompt))],
            [theme.muted('Completion'), theme.muted(fmt(data.tokens?.completion))],
            [theme.color('info', 'Total'), theme.color('info', fmt(data.tokens?.total))]
        ];

        const leftLines = [theme.color('info', 'METRICS')];
        for (const row of metricRows) {
            if (row === null) {
                leftLines.push(theme.muted('─'.repeat(leftW)));
                continue;
            }
            leftLines.push(
                padEndVisible(row[0], labelW) + '  ' + padStartVisible(row[1], valueW)
            );
        }

        const byeLines = renderByeByeEmblem(frame, theme);
        const byeW = getByeByeEmblemWidth();

        const modelUsageKeys = Object.keys(data.modelUsage || {});
        let modelBlock = '';
        let modelBlockW = 0;
        if (modelUsageKeys.length > 0) {
            ...
            const header =
                '  ' +
                padEndVisible(theme.muted('Model'), mNameW) + '  ' +
                padStartVisible(theme.muted('Reqs'), mReqW) + '  ' +
                padStartVisible(theme.muted('In'), mNumW) + '  ' +
                padStartVisible(theme.muted('Out'), mNumW) + '  ' +
                padStartVisible(theme.muted('Total'), mNumW);

            const modelLines = modelUsageKeys.map((key) => {
                const item = data.modelUsage[key] || {};
                const name = truncateVisible(item.name || key, mNameW);
                return (
                    '  ' +
                    padEndVisible(theme.color('text', name), mNameW) + '  ' +
                    padStartVisible(theme.color('text', String(item.requests || 1)), mReqW) + '  ' +
                    padStartVisible(theme.muted(fmt(item.prompt)), mNumW) + '  ' +
                    padStartVisible(theme.muted(fmt(item.completion)), mNumW) + '  ' +
                    padStartVisible(theme.color('info', fmt(item.total)), mNumW)
                );
            });

            modelBlock =
                theme.color('info', '  MODEL USAGE') + '\n' +
                header + '\n' +
                theme.muted('  ' + '─'.repeat(mNameW + mReqW + mNumW * 3 + 8)) + '\n' +
                modelLines.join('\n');
            modelBlockW = Math.max(...modelBlock.split('\n').map((l) => stripAnsi(l).length));
        }

        // Place compact logo in the *center* of the gap after metrics (not flush-right)
        const bandInnerW = Math.max(leftW + 2 + byeW + 4, modelBlockW - 2, leftW + 16);
        const gapW = Math.max(byeW + 4, bandInnerW - leftW);
        const logoLead = Math.max(2, Math.floor((gapW - byeW) / 2));
        const topPad = Math.max(0, Math.floor((leftLines.length - byeLines.length) / 2));
        const bandH = Math.max(leftLines.length, topPad + byeLines.length);
        const band = [];
        for (let i = 0; i < bandH; i++) {
            const L = padEndVisible(leftLines[i] || '', leftW);
            const byeIdx = i - topPad;
            const R = (byeIdx >= 0 && byeIdx < byeLines.length)
                ? padEndVisible(byeLines[byeIdx], byeW)
                : ' '.repeat(byeW);
            band.push('  ' + L + ' '.repeat(logoLead) + R);
        }
        ...
        return theme.box(body, {
            title: theme.color('info', ' SESSION RECAP '),
            titleAlignment: 'center',
            padding: { top: 0, bottom: 0, left: 1, right: 1 },
            margin: { top: 0, bottom: 0 },
            borderStyle: theme.frame.boxen || 'round',
            width: Math.min((process.stdout.columns || 80) - 2, contentW + 4)
        });
    }

    /**
     * Display session recap and token usage metrics on session exit (or Settings view).
    ...
    static showInteractionSummary(sessionData, options = {}) {
        ...
        const afterAnimation = Boolean(options.afterAnimation);

        if (!viewOnly && !afterAnimation) {
            console.log(theme.muted('\n  Session complete.\n'));
        }

        console.log(TerminalUI.buildSessionRecapBox(sessionData, theme, 0));

        if (!viewOnly) {
            console.log(theme.muted('\n  ') + theme.color('text', MOTTO) + '\n');
            if (canAnimateExit() && !options.skipAnimation) {
                console.log(theme.success('  ✓  Ready for the next session.\n'));
            }
        }
    }
    ...
    /**
     * Professional boxed agent response — titled frame, full-width rule, wrapped body.
     * Nested code boxes are pre-sized; box-drawing lines are never wrapAnsi'd (avoids border bleed).
     */
    static printAgentResponse(renderedMarkdown, config = null) {
        const theme = getTheme(config && config.getBrandingFont ? config.getBrandingFont() : getDefaultThemeId());
        const wrapAnsi = require('wrap-ansi');
        const { getResponseLayout } = require('./renderer');
        const rawBody = String(renderedMarkdown || '').trim();
        const { outerW, contentW } = getResponseLayout();

        const title =
            theme.color('info', 'LORAPOK') +
            theme.muted('  ·  ') +
            theme.muted('response');
        const rule = theme.muted('─'.repeat(contentW));

        const BOX_ART = /[┌┐└┘│╭╮╰╯╔╗╚╝║═┏┓┗┛┃─]/;

        let body;
        if (!rawBody) {
            body = theme.muted('(empty response)');
        } else {
            body = rawBody
                .split('\n')
                .map((line) => {
                    ...
                    // Never wrap pre-sized code/table frames — wrapAnsi splits SGR and bleeds color
                    if (BOX_ART.test(plain)) return line;
                    if (plain.length <= contentW) return line;
                    return wrapAnsi(line, contentW, { hard: false, trim: false, wordWrap: true });
                })
                .join('\n')
                .replace(/\n{3,}/g, '\n\n');
        }
    ...
    static showDiff(filePath, oldContent, newContent) {
        ...
        for (let i = 0; i < maxLines; i++) {
            const isMatch = oldLines[i] === newLines[i];
            
            if (!currentBlock || currentBlock.isMatch !== isMatch) {
                currentBlock = { isMatch, startIdx: i, lines: [] };
            ...
            if (block.isMatch) {
                if (numLines <= 3) {
                    for (const line of block.lines) {
                        if (line.oldLine !== undefined) output += chalk.gray(`  L${line.idx + 1}: ${line.oldLine}\n`);
                    }
                } else {
                    const first = block.lines[0];
                    const last = block.lines[numLines - 1];
                    if (first.oldLine !== undefined) output += chalk.gray(`  L${first.idx + 1}: ${first.oldLine}\n`);
                    output += chalk.gray(`  ... [ ${numLines - 2} unchanged lines hidden ] ...\n`);
                    if (last.oldLine !== undefined) output += chalk.gray(`  L${last.idx + 1}: ${last.oldLine}\n`);
                }
            } else {
                if (numLines <= 4) {
                    for (const line of block.lines) {
                        if (line.oldLine !== undefined) output += chalk.red(`- L${line.idx + 1}: ${line.oldLine}\n`);
                        if (line.newLine !== undefined) output += chalk.green(`+ L${line.idx + 1}: ${line.newLine}\n`);
                    }
                } else {
                    const first = block.lines[0];
                    const last = block.lines[numLines - 1];
                    
                    if (first.oldLine !== undefined) output += chalk.red(`- L${first.idx + 1}: ${first.oldLine}\n`);
                    if (first.newLine !== undefined) output += chalk.green(`+ L${first.idx + 1}: ${first.newLine}\n`);
                    
                    let oldHidden = 0;
                    let newHidden = 0;
                    for (let i = 1; i < numLines - 1; i++) {
                        if (block.lines[i].oldLine !== undefined) oldHidden++;
                        if (block.lines[i].newLine !== undefined) newHidden++;
                    }
                    
                    output += chalk.cyan(`  ... [ ${newHidden} additions, ${oldHidden} deletions hidden ] ...\n`);
                    
                    if (last.oldLine !== undefined) output += chalk.red(`- L${last.idx + 1}: ${last.oldLine}\n`);
                    if (last.newLine !== undefined) output += chalk.green(`+ L${last.idx + 1}: ${last.newLine}\n`);
                }
            }
        }

        console.log(boxen(output.trimEnd(), {
            padding: 1,
            borderStyle: 'round',
            borderColor: 'cyan',
            title: 'Code Viewport',
